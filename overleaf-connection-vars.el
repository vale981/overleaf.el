;;; overleaf-connection-vars.el --- Shared variable and customize definitions  -*- lexical-binding: t; -*-
...
(eval-and-compile
  (defcustom overleaf-connection-keymap-prefix "C-c C-o"
    "The prefix for dotcrafter-mode key bindings."
    :type 'string
    :group 'overleaf-connection-mode))

(defvar overleaf-connection-cookies nil
...
`overleaf-connection-read-cookies-from-file'.

The cookies are most easily obtained from the developer tools in the
browser.")

(defun overleaf-connection-read-cookies-from-file (file)
  "Return a cookie saving function to load the cookie-string from FILE.
To be used with `overleaf-connection-cookies'."
  #'(lambda ()
      (with-temp-buffer
        (insert-file-contents (expand-file-name file))
        (string-trim (buffer-string)))))

(defvar overleaf-connection-save-cookies #'(lambda (cookies)
                                             (setq overleaf-connection-cookies cookies))
...
in a way that `overleaf-connection-cookies' can access it.  The default
implementation simply sets `overleaf-connection-cookies' to the string value.
Another possibility is to store them into a gpg encrypted file.  See
`overleaf-connection-save-cookies-to-file'.")

(defun overleaf-connection-save-cookies-to-file (file)
  "Return a cookie saving function to save the cookie-string to FILE.
To be used with `overleaf-connection-save-cookies'."
  #'(lambda (cookies)
      (with-temp-file file
        (insert cookies))))

(defcustom overleaf-connection-url "https://www.overleaf.com"
  "The url of the overleaf server."
  :type 'string
  :group 'overleaf-connection-mode)

(defcustom overleaf-connection-flush-interval .5
  "The idle-timer delay to flush the edit queue."
  :type 'float
  :group 'overleaf-connection-mode)

(defcustom overleaf-connection-message-interval .5
  "The interval for the timer that syncs changes to overleaf."
  :type 'float
  :group 'overleaf-connection-mode)

(defcustom overleaf-connection-debug nil
  "Whether to log debug messages."
  :type 'boolean
  :group 'overleaf-connection-mode)

(defvar-local overleaf-connection-auto-save nil
  "Whether to auto-save the buffer each time a change is synced.")

(defvar-local overleaf-connection-project-id nil
...
(defvar-local overleaf-connection-document-idea nil
...
(defvar-local overleaf-connection-track-changes nil
  "Whether or not to track changes in overleaf.")


(defvar-local overleaf-connection--is-overleaf-change nil
  "Is set to t if the current change in the buffer comes from overleaf.
Used to inhibit the change detection.")

(defvar-local overleaf-connection--force-close nil
  "If t the connection will not be reestablished upon disconnection.")

(defvar-local overleaf-connection--websocket nil
  "The websocket instance connected to overleaf.")

(defvar-local overleaf-connection--flush-edit-queue-timer nil
  "A timer that flushes the edit queue.

See `overleaf-connection--edit-queue'.")

(defvar-local overleaf-connection--message-timer nil
  "A timer that sends edits from the message queue if it isn't empty.

See `overleaf-connection--message-queue'.")

(defvar-local overleaf-connection--edit-queue '()
  "A list of edits that can be send in one go.")

(defvar-local overleaf-connection--send-message-queue '()
  "A list of messages containing edits that have yet to be synced to overleaf.

See `overleaf-connection--message-timer'.")

(defvar-local overleaf-connection--edit-in-flight nil
  "If non-nil, we still await the acknowledgment from overleaf.")

(defvar-local overleaf-connection--doc-version -1
  "Current version of the document.")

(defvar-local overleaf-connection--mode-line ""
  "Contents of the mode-line indicator.")

(defvar overleaf-connection--ws-url->buffer-table (make-hash-table :test #'equal)
  "A hash table associating web-sockets to buffers.")

(defvar overleaf-connection--buffer nil
  "The current overleaf buffer (used in lexical binding).")

(defvar-local overleaf-connection--buffer-before-edit-queue ""
  "The contents of the buffer before any edits were queued.")

(defvar-local overleaf-connection--last-good-state nil
  "The last received overleaf update.")

(defvar-local overleaf-connection--history nil
...
(defvar-local overleaf-connection-history-buffer-length 50
  "How many past versions of the buffer to keep in memory.")

(defvar-local overleaf-connection--recent-updates nil
  "An alist of recent updates received from overleaf.

An alist that contains the `overleaf-connection-update-buffer-length'
recent updates.  It has elements of the form `((from-version
. (to-version . update)) ...)'.")

(defvar-local overleaf-connection-update-buffer-length 50
  "How many past updates of the buffer to keep in memory.")

(defvar overleaf-connection--current-cookies nil
  "The current cookies so we don't have to read them every time.")

(defvar-local overleaf-connection--receiving nil
  "When t we are currently in the process of receiving and processing an update.")

(cl-defstruct overleaf-connection--queued-message
...
(cl-defstruct overleaf-connection--update
...
HASH is a hash obtained from `overleaf-connection--get-hash' after the
edit is performed.
...
(provide 'overleaf-connection-vars)
;;; overleaf-connection-vars.el ends here
