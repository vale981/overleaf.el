...
live-edit $\LaTeX{}$ files on [[https://github.com/overleaf/overleaf][overleaf]]. Changes made offline can be synced to overleaf using =ediff=.
...
On GNU/Linux, the default =<firefox-folder>= is typically =~/.mozilla/firefox/=.
On macOS, =<firefox-folder>= is typically =~/Library/Application Support/Firefox=.
...
- ~'centralized~: Saves all ancestor backups in a dedicated directory at ~~/.emacs.d/overleaf-ancestors/~.
