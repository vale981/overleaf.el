...
~overleaf-cookies~ to a function that returns the cookie string.
...
* Usage
After having set things up, just use ~overleaf-connect~ or ~overleaf-auto-connect~ to connect to overleaf (either from an empty buffer, or in a buffer with the appropriate file-local variables set from a previous session).
